...
public class ForecastPointScraper {
	...
	// etc.
	static JSONArray scrapeAllForecastPointsFromCoordsNow(double west, double east, double south, double north) {
		return WebWorm.getPageJSONArray(250, "https://videscentrs.lvgmc.lv/data/weather_points_forecast?laiks=" + TimeManager.DATE
				+ TimeManager.TIME.split(":")[0] + "00&bounds=POLYGON%20((%20" + west + "%20" + south + "%2C%20" + east + "%20"
	...
	static JSONArray getAllPointsOfResolution(int n) { // 1 - lowest, 10 - highest resolution
		if (n < 1 || n > 10)
			return null;
		if (n == 1) {
			return scrapeAllForecastPointsFromCoordsNow(LV_WEST, LV_EAST, LV_SOUTH, LV_NORTH);
		} else {
			...
			for (int i = 0; i < rowCount; i++) {
				for (int j = 0; j < rowCount; j++) {
					...
					jsonArr.putAll(scrapeAllForecastPointsFromCoordsNow(west, east, south, north));
				}
			}
			return jsonArr;
		}
	...
	static String[][] getFullForecastTableArrayForPlaceAndTime(String townName, String regionName, String dateTime) {
		int pointID = Main.db.getPointIDFromName(townName, regionName);
